...
var ID_FOLDER_ENVIOS_PDF = '1iJSvRwT-lsXAp8xIkSHRH5XZ73wrqO54';
...
function subirPDFaDrive(base64Data, nombreArchivo) {
  try {
    ...
    return 'https://drive.google.com/file/d/' + archivo.getId() + '/view?usp=sharing';
  } catch(e) {
    throw new Error('Error subiendo PDF a Drive: ' + e.message);
  }
}

// ── Sube PDF a Drive Y guarda filas en una sola llamada ──
function guardarFilasPDFConDrive(filas, base64Data, nombreArchivo) {
  var urlPdf = '';
  if (base64Data) {
    try {
      var blob    = Utilities.newBlob(Utilities.base64Decode(base64Data), 'application/pdf', nombreArchivo);
      var archivo = DriveApp.createFile(blob); // Crear en raíz (siempre funciona)
      archivo.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
      try {
        // Intentar mover al folder destino
        var folder = DriveApp.getFolderById(ID_FOLDER_ENVIOS_PDF);
        folder.addFile(archivo);
        DriveApp.getRootFolder().removeFile(archivo);
      } catch(eFol) {
        // Si falla el move, el archivo queda en raíz — no importa, ya tenemos URL
      }
      urlPdf = 'https://drive.google.com/file/d/' + archivo.getId() + '/view?usp=sharing';
    } catch(e) {
      throw new Error('DRIVE_ERROR: ' + e.message);
    }
  }
  var ss    = SpreadsheetApp.openById(ID_HOJA_ENVIADO_PDF);
  var sheet = ss.getSheetByName('ENVIADO');
  ...
  filas.forEach(function(f) {
    ...
    sheet.appendRow([
      ...
      f.pedido         || '',  // F: PEDIDO
      "'" + (f.codigo || ''),  // G: CODIGO (apóstrofo fuerza texto en Sheets)
      f.descripcion    || '',  // H: DESCRIPCION
      f.familia        || '',  // I: FAMILIA
      f.kilos          || 0,   // J: KILOS
      f.piezas         || 0,   // K: PIEZAS
      f.comentario     || '',  // L: COMENTARIOS
      f.envio          || '',  // M: ENVIO
      f.colada         || '',  // N: COLADA
      f.lote           || '',  // O: LOTE
      f.chofer         || '',  // P: CHOFER
...
////////////////////////////////////////////////////////////////////////////////////////////
//  EMBARQUES_CODIGO_GS — Pegar esto DEBAJO del contenido de EmbarquesGS en el nuevo script
//  Contiene: variables globales, doGet, renderPage y funciones necesarias de Código.txt
////////////////////////////////////////////////////////////////////////////////////////////
...
var ID_CARPETA_INSUMOS = "1Lz3LZWGkp0jAhJuPCYeSvjqphSQMGAh7";
var ID_HOJA_EMBARQUES  = "1RKi09zpQ3KMa_JLUINYJysDOFRi3tM2M2a8JW8Qy7gk"; // ajusta si es diferente


// ══════════════════════════════════════════════════════════════════════
//  doGet — Sirve los HTMLs del módulo de Embarques
// ══════════════════════════════════════════════════════════════════════
function doGet(e) {
  try {
    var v = (e && e.parameter && e.parameter.v) ? e.parameter.v : '';
    var filename = 'MainApp_EmbarquesHTML';
    var title    = 'EMBARQUES';
    if (v === 'existencia') {
...
// ══════════════════════════════════════════════════════════════════════
//  USUARIOS — Login y administración
// ══════════════════════════════════════════════════════════════════════
...
function cambiarPasswordUsuario(nombre, nuevaPassword) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      if (String(data[i][1]).trim().toUpperCase() === String(nombre).trim().toUpperCase()) {
        sheet.getRange(i + 1, 3).setValue(String(nuevaPassword).trim());
        return "OK";
      }
    }
    return "Error: usuario no encontrado";
  ...
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      if (String(data[i][1]).trim().toUpperCase() === String(obj.nombre).trim().toUpperCase()) {
        ...
        // Conservar permisos que NO son de Embarques (E_)
        var otrosPerms = permActuales.filter(function(p){ return !p.startsWith("E_"); });
        var nuevosE = String(obj.permisos || "")
          .split(",").map(function(p){ return p.trim(); })
          .filter(function(p){ return p.startsWith("E_"); });
        var permFinal = otrosPerms.concat(nuevosE).join(",");
...
function eliminarUsuario(nombre) {
  try {
    ...
    for (var i = data.length - 1; i >= 1; i--) {
      if (String(data[i][1]).toUpperCase() === nombre.toUpperCase()) {
        sheet.deleteRow(i + 1);
        return "OK";
      }
    }
...
function guardarRegistrosCirculante(registros) {
  ...
  var filas = registros.map(function(r) {
    ...
    return [
      nextId++, fechaObjeto, r.envio, r.codigo, r.descripcion, r.diametro, r.largo, r.acero,
      r.pedido, r.origen, r.cantidad, r.unidad, k, kr, mesFila, anioFila,
      dif, r.comentarios, r.movimiento, "", ahora
    ];
  });
...
function eliminarRegistroCirculante(id) {
  try {
    var ss    = SpreadsheetApp.openById(ID_HOJA_OM);
    var sheet = ss.getSheetByName("CIRCULANTE");
    var data  = sheet.getDataRange().getValues();
...
function crudCatalogo(item) {
  ...
  if (item.id === "NUEVO") {
    ...
    sh.appendRow([maxId + 1, item.codigo, item.descripcion, item.diametro, item.largo, item.acero, item.unidad]);
  } else {
    for (var i = 1; i < data.length; i++) {
...
function verificarSellosDuplicados(sellosNuevos) {
  ...
  return sellosNuevos.filter(sello => {
...
function guardarEntradasMP(registros) {
  ...
  const rowsToAppend = registros.map((r, index) => {
    ...
    return [
      nextId + index, r.proveedor, fechaFormateada, r.diametro, r.acero,
      r.n_rollo, r.kilos, r.colada, r.sello, r.n_remision, r.consecutivo, "ACTIVO"
    ];
  });
...
function procesarSalidasMP(cambios) {
  ...
  cambios.forEach(cambio => {
    const rowIndex = data.findIndex(row => row[0] == cambio.id);
    if (rowIndex !== -1) {
      const sheetRow = rowIndex + 1;
      sheet.getRange(sheetRow, 12).setValue("BAJA");
      sheet.getRange(sheetRow, 13).setValue(cambio.fechaSalida);
      sheet.getRange(sheetRow, 15).setValue(data[rowIndex][6]);
...
function getDatosReporteMensual(mes, anio) {
  try {
    ...
    return data.map(r => ({
      ...
      fechaEntrada: formatFecha(r[2]),
      diametro:     String(r[3]),
      acero:        String(r[4]),
      kilos:        parseFloat(r[6]) || 0,
      sello:        String(r[8]),
      estado:       String(r[11]),
      fechaSalida:  formatFecha(r[12]),
      kilosSalida:  parseFloat(r[14]) || 0
    }));
  } catch (e) { return []; }
}
...
function getConsultasMP(tipo, fInicio, fFin) {
  try {
    ...
    return data.slice(1).filter(r => {
      ...
      return fechaRow.getTime() >= tInicio && fechaRow.getTime() <= tFin;
    }).map(r => ({
      id: r[0], proveedor: r[1],
      fecha: r[2] instanceof Date ? Utilities.formatDate(r[2], "GMT-6", "dd/MM/yyyy") : r[2],
      diametro: r[3], acero: r[4], n_rollo: r[5], kilos: r[6], colada: r[7],
      sello: r[8], n_remision: r[9], consecutivo: r[10], estado: r[11],
      fecha_salida: r[12] instanceof Date ? Utilities.formatDate(r[12], "GMT-6", "dd/MM/yyyy") : r[12],
      kilos_salida: r[14], comentarios: r[16]
    }));
  } catch (e) { return []; }
}
...
function actualizarEstadoMP(id, nuevoEstado, infoSalida, nombreUsuario) {
  ...
  if (nuevoEstado === "ACTIVO") {
    sheet.getRange(sheetRow, 12).setValue("ACTIVO");
    sheet.getRange(sheetRow, 13).clearContent();
    sheet.getRange(sheetRow, 15).clearContent();
